...
static void setupTx(bool retry) {
	...
	if (retry) {
		...
		if (servicePacket.size) {
			...
			if (has_safelink) {
				txPackets[txq_tail].data[0] = (txPackets[txq_tail].data[0]
						& 0xf3) | curr_down << 2;
			}
...
void crazyflieAddress() {
	...
	// Using logical address 0 so only BASE0 and PREFIX0 & 0xFF are used
	NRF_RADIO->PREFIX0 = 0xC4C3C200UL
	| (bytewise_bitswap(crazyflie_address >> 32) & 0xFF);// Prefix byte of addresses 3 to 0
	NRF_RADIO->PREFIX1 = 0xC5C6C7C8UL;  // Prefix byte of addresses 7 to 4
NRF_RADIO->BASE0 = bytewise_bitswap((uint32_t) crazyflie_address); // Base address for prefix 0
}
